	...
	useEffect(() => {
		...
		const loop = () => {
			if (timeRef.current !== null) {
				timeRef.current.textContent = `${formatTime(playbackClock.currentTime())} / ${formatTime(playbackClock.maxTime)}`;
			}
		...
		<footer className="pointer-events-auto absolute inset-x-0 bottom-0 z-20 flex flex-col gap-1 border-t border-zinc-800/60 bg-zinc-950/80 px-3 pb-2 pt-1 backdrop-blur">
			...
			<div className="flex items-center gap-2">
				...
				<Button
					...
					className="bg-[#ff66ab] text-zinc-950 hover:bg-[#ff87bc]"
					onClick={() => setPlaying(!playing)}
				>
					{playing ? <Pause /> : <Play />}
				</Button>
				<Button size="icon" variant="ghost" aria-label="previous frame" onClick={() => stepFrame(-1)}>
					<StepBack />
				</Button>
				<Button size="icon" variant="ghost" aria-label="next frame" onClick={() => stepFrame(1)}>
					<StepForward />
				</Button>
...
export function HudReadout() {
	...
	return (
		<>
			<div
				ref={comboRef}
				className="pointer-events-none absolute bottom-24 left-3 z-10 text-3xl font-bold tabular-nums text-white/90"
			>
				0x
			</div>
			<div
				ref={accuracyRef}
				className="pointer-events-none absolute right-3 top-12 z-10 text-lg font-semibold tabular-nums text-white/80"
			>
				100.00%
			</div>
		</>
