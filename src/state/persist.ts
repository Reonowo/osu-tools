...
import type { IpcError, OverlaySettings } from "../lib/scene-types";
import type { ViewerState } from "./store";

export type PrefsSaver = (volume: number, overlays: OverlaySettings) => Promise<unknown>;
...
export function installPrefsPersistence(
  ...
  const flush = () => {
    const { volume, overlays } = store.getState();
    save(volume, overlays).catch((e: unknown) => {
  ...
  const unsubscribe = store.subscribe((state, prev) => {
    // reference equality on overlays: setOverlay always builds a new object,
    // and every other store write (scene installs, playback, rate) leaves
    // both fields untouched, so those never schedule a save
    if (state.volume === prev.volume && state.overlays === prev.overlays) return;
