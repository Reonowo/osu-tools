...
export function describeIpcError(e: IpcError): { title: string; detail: string; recovery: Recovery } {
  switch (e.kind) {
    ...
    case "unsupportedMode":
      return { title: "unsupported game mode", detail: `${e.mode} isn't supported; only osu! standard`, recovery: null };
    case "resourceLimit":
      return { title: "file exceeds safety limits", detail: `${e.cap}: ${e.actual} > ${e.limit}`, recovery: null };
    case "io":
