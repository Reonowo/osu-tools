...
import {
  Container, Geometry, Mesh, RenderTexture, Shader, Sprite, Texture, type Renderer,
} from "pixi.js";
...
export class SliderBodyRenderer {
  ...
  constructor(renderer: Renderer, slider: RenderSlider, accent: Rgba, csScale: number) {
    ...
    const scale = Math.min(
      SUPERSAMPLE,
      MAX_TEXTURE_DIM / Math.max(this.bounds.width, this.bounds.height, 1),
    );
    ...
    this.compositeGeometry = new Geometry({
      attributes: {
        aPosition: new Float32Array([
          0, 0, this.bounds.width, 0, this.bounds.width, this.bounds.height, 0, this.bounds.height,
        ]),
