...
const NESTED_EVENT_KIND = {
  tick: "sliderTick", repeat: "sliderRepeat", tail: "sliderTail",
} as const;
...
export class SliderDrawable implements ObjectDrawable {
  ...
  constructor(ctx: RenderContext, objectIndex: number) {
    ...
    for (const nested of slider.nested) {
      ...
      if (nested.kind === "tick") {
        ...
        this.pieces.push({ nested, view, alpha: tracks.alpha, scale: tracks.scale, arrow: null, appear: nested.time - nested.preempt, result, aimed: false });
      } else {
        ...
        this.pieces.push({ nested, view, alpha: tracks.alpha, scale: null, arrow, appear: nested.time - nested.preempt, result, aimed: false });
      }
    }
    ...
    const changes = st.trackingStateChanges(
      ctx.scene.frames,
      {
        ...
        scale: this.planScale, duration: slider.duration,
      },
      slider, slider.spanCount,
    );
  ...
  update(t: number): void {
    ...
    const headHit = this.simulated
      ? !this.headHit.miss && t >= this.headHit.time
      : t >= this.obj.startTime;
