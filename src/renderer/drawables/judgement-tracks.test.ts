...
import {
  GRADE_COLOURS, judgementSpecs, resultTracks, ringExplosion, tickMissTracks,
...
describe("judgement specs", () => {
    ...
    const scene = testScene({
      renderPlan: {
        ...
        objects: [{
          ...
          endTime: 1500,
          kind: {
            type: "slider", vertices: [0, 0, 100, 0], cumulativeLengths: [0, 100], distance: 100,
            segmentEnds: [1], repeatCount: 0, spanCount: 1, spanDuration: 500, duration: 500,
            endPosition: [200, 100], snakeInDuration: 200,
            nested: [
              { kind: "head", spanIndex: 0, time: 1000, position: [100, 100], pathProgress: 0, preempt: 600, fadeIn: 400 },
              { kind: "tick", spanIndex: 0, time: 1250, position: [150, 100], pathProgress: 0.5, preempt: 500, fadeIn: 150 },
              { kind: "tail", spanIndex: 0, time: 1500, position: [200, 100], pathProgress: 1, preempt: 600, fadeIn: 400 },
            ],
          },
        }],
      },
      simulation: {
        ...
        events: [
          { time: 1000, objectIndex: 0, kind: { type: "sliderHead", hit: true }, comboAfter: 1, accuracyAfter: 1 },
          { time: 1250, objectIndex: 0, kind: { type: "sliderTick", hit: false }, comboAfter: 0, accuracyAfter: 1 },
          { time: 1500, objectIndex: 0, kind: { type: "sliderTail", hit: true }, comboAfter: 1, accuracyAfter: 1 },
          { time: 1500, objectIndex: 0, kind: { type: "sliderAggregate", grade: "ok" }, comboAfter: 1, accuracyAfter: 0.9 },
        ],
  ...
  test("hit slider ticks/repeats produce nothing even when nested data matches", () => {
    ...
    const scene = testScene({
      ...
      simulation: {
        status: "authoritative",
        events: [{ time: 1250, objectIndex: 0, kind: { type: "sliderTick", hit: true }, comboAfter: 1, accuracyAfter: 1 }],
  ...
  test("spinner finals pop at the playfield centre", () => {
    const scene = testScene({
      simulation: {
        status: "authoritative",
        events: [{ time: 980, objectIndex: 0, kind: { type: "spinnerFinal", grade: "great" }, comboAfter: 1, accuracyAfter: 1 }],
    ...
    const scene = testScene({
      simulation: {
        ...
        events: [
          ...
          { time: 980, objectIndex: 0, kind: { type: "circle", grade: "meh" }, comboAfter: 1, accuracyAfter: 1 },
        ],
