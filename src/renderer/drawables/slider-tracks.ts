...
export function ballTracks(
  obj: { startTime: number },
  endTime: number,
): { alpha: Track[]; iconScale: Track[] } {
  return {
    // drawableslider.cs (updatestarttimestatetransforms: ball.fadein()) +
    // argonsliderball.cs:90-103 (the actual alpha/scale timings)
    alpha: [
      jump(obj.startTime, 0),
      tween(obj.startTime, 200, 0, 1, outQuint),
      tween(endTime, 50, 1, 0, outQuint),
    ],
    iconScale: [
      tween(obj.startTime, 200, 0, 1, outElasticHalf),
      tween(endTime, 200, 1, 0.9, outQuint),
    ],
  };
}
...
export function repeatTracks(
  ...
  alpha.push(resolved === "hit"
    ? tween(nested.time, animDuration, 1, 0, out)
    : tween(nested.time, animDuration, 1, 0));
  return { alpha };
}
...
export function trackingStateChanges(
  ...
  for (const frame of frames) {
    ...
    const tracking = (isLeft(state.buttons) || isRight(state.buttons)) &&
      dx * dx + dy * dy <= followRadius * followRadius;
    if (tracking !== last) {
