...
export function generateFollowPoints(objects: RenderObject[], globalScale: number): FollowPointSpec[] {
  ...
  for (let i = 0; i < objects.length - 1; i++) {
    ...
    for (let d = Math.trunc(FOLLOW_POINT_SPACING * 1.5); d < distance - FOLLOW_POINT_SPACING; d += FOLLOW_POINT_SPACING) {
  ...
  update(t: number): void {
    ...
    reconcileActiveDrawables(
      ...
      (index) => {
        ...
        const sprite = new Sprite(this.ctx.textures.canvasTexture(32, "followpoint", (c, size) => {
          c.strokeStyle = "#fff";
          c.lineWidth = 4;
          c.lineCap = "round";
          const draw = (x: number) => {
