...
import {
  BORDER_THICKNESS, INNER_FILL_SIZE, INNER_GRADIENT_SIZE, OBJECT_RADIUS, OUTER_GRADIENT_SIZE,
} from "../../engine/argon";
...
export class ArgonCirclePiece {
  ...
  constructor(ctx: RenderContext, accent: Rgba, indexInCombo: number, withOuterFill: boolean) {
    ...
    this.innerGradient = sprite(
      t.gradientCircleTexture(INNER_GRADIENT_SIZE * TEXTURE_SCALE, `inner:${key}`, darken(accent, 0.5), darken(accent, 0.6)),
      INNER_GRADIENT_SIZE,
    );
    ...
    for (const child of [
      this.outerFill, this.outerGradient, this.outerGradientWhite, this.innerGradient,
      this.innerFill, this.number, this.flash, this.border,
    ]) {
  ...
  apply(tracks: CircleTracks, t: number): void {
    ...
    this.outerGradientWhite.scale.set((OUTER_GRADIENT_SIZE / this.outerGradientWhite.texture.width) * gradientScale);
