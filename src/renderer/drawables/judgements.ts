...
import {
  ...
  type JudgementSpec, type RingSpec,
...
export class JudgementsDrawable implements ObjectDrawable {
  ...
  constructor(private readonly ctx: RenderContext) {
    ...
    this.tracker = new ActiveSetTracker(this.specs.map((s) => ({
      appear: s.time,
  ...
  private buildResult(spec: JudgementSpec): Popup {
    ...
    if (spec.grade !== "miss") {
      for (const ring of ringExplosion(spec.grade, spec.seed)) {
        const texture = this.ctx.textures.ringTexture(ring.size * TEXTURE_SCALE, RING_THICKNESS * TEXTURE_SCALE);
