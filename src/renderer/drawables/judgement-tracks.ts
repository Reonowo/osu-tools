...
export function judgementSpecs(scene: LoadedScene): JudgementSpec[] {
  ...
  scene.simulation.events.forEach((event, seed) => {
    ...
    switch (kind.type) {
      case "circle":
        specs.push({ time: event.time, x: obj.position[0], y: obj.position[1], grade: kind.grade, style: "text", seed });
        break;
      ...
      case "sliderRepeat": {
        ...
        if (nested !== null) {
          specs.push({ time: event.time, x: nested.position[0], y: nested.position[1], grade: "miss", style: "tickMiss", seed });
        }
        break;
      }
      default:
