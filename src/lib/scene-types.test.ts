...
import type {
  IpcError, JudgementKindDto, LoadedSceneWarning, RenderKind, SimulationDto,
} from "./scene-types";
...
describe("scene contract mirror", () => {
    ...
    const sims: SimulationDto[] = [
      {
        ...
        events: [{
          time: 1030, objectIndex: 0,
          kind: { type: "circle", grade: "meh" },
          comboAfter: 1, accuracyAfter: 50 / 300,
        }],
  ...
  test("render kinds narrow on type", () => {
    ...
    const kinds: RenderKind[] = [
      ...
      {
        type: "slider",
        vertices: [0, 0, 100, 0], cumulativeLengths: [0, 100], distance: 100,
        segmentEnds: [1], repeatCount: 0, spanCount: 1, spanDuration: 500,
        duration: 500, endPosition: [200, 100], snakeInDuration: 200,
        nested: [{ kind: "head", spanIndex: 0, time: 2000, position: [100, 100], pathProgress: 0, preempt: 600, fadeIn: 400 }],
      },
