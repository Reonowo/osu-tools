		...
		for (const c of fixture.cases) {
			...
			for (const r of c.path_to_progress) {
				...
				r.vertices.forEach(([x, y], i) => {
					...
					expect(Math.abs(ours[i * 2 + 1] - y), `${c.name} [${r.p0},${r.p1}] v${i} y`).toBeLessThanOrEqual(
						1e-4
					);
				});
			}
...
describe("ball position parity: rust geometry × lazer ball_samples", () => {
	const stems = ["old-format-v4", "slider-zoo-v14", "spinners-combos-od10", "stacking-v14", "v7-tick-multiplier"];
