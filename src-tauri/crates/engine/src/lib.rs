...
//! | `formats::binary` | `osu.Game/IO/Legacy/SerializationReader.cs` -- the byte-level reads the binary osu! formats share (bounded takes, the little-endian fixed-width numbers, ULEB128, the tag-prefixed osu! string), lifted out of the `.osr` codec once a second codec needed them. crate-private, with the error CONSTRUCTOR injected so a failure keeps the name of the file it happened in: the same short read is a replay-parse error under [`formats::osr`] and a listing-parse error under [`formats::stable_listing`] |
//! | [`formats::osr`] | byte framing follows `LegacyScoreDecoder.cs`/`LegacyScoreEncoder.cs`; decompression tolerance is checked against `SharpCompress.Compressors.LZMA.LzmaStream`'s actual runtime behaviour, which is looser than lazer's own encoder output. its header-only entry point ([`formats::osr::scan_osr_header`]) ports nothing further -- it stops where the framing puts the frame payload, so a folder of thousands lists without decompressing one, and `tests/osr_header_scan.rs` pins it against the full decoder on every fixture and corpus replay |
...
//! | [`formats::stable_listing`] | not a port of a lazer source file -- lazer never reads `osu!.db` at all (its stable import walks the Songs directory). the framing is stable's own, read through the `formats::binary` primitives that DO port `SerializationReader.cs`, and pinned by real client bytes in `fixtures/stable/<version>/` on both sides of the 20250107 star-rating layout change. it keeps md5, folder and file name, plus the six naming fields a replay browser row is titled and searched by, and walks everything else |
//! | [`formats::local_scores`] | not a port of a lazer source file, and further from one than the listing beside it: lazer has no equivalent of `scores.db` and never reads it. what IS ported is the framing underneath -- the same `formats::binary` primitives, hence `SerializationReader.cs` -- while the field order is stable's own, pinned by a real client's bytes in `fixtures/stable/<version>/scores.db`. it keeps one row per local play and walks the group keys, the empty life-bar string and the null payload sentinel past |
...
//! | [`simulation`]`::*` | `legacyhitpolicy.cs` (classic note lock); `drawablehitcircle.cs`/`drawablesliderhead.cs`/`drawableslider.cs:293-315` (classic circle/slider-head/aggregate judgement); `sliderinputmanager.cs` (tracking state machine, key restriction, `postprocessheadjudgement`); `spinnerrotationtracker.cs`/`spinnerspinhistory.cs`/`drawablespinner.cs` (spinner presentation rotation and ticks); danser-go `8331b0ff` `rulesets/osu/spinner.go` (stable disc scoring and final grade); `hitresult.cs`/`osulegacyscoresimulator.cs` (combo/count semantics, with the one deliberate divergence noted below) |
//! | [`simulation::native`] | the native profile's walk, the second behind the rules profile: `framestabilitycontainer.cs`/`framedreplayinputhandler.cs:113-160` (the update cadence: every frame, the important-section rule, one update per duplicated timestamp); `starttimeorderedhitpolicy.cs` (the playfield's default note lock: a press blocked before the last alive circle's start, an out-of-order hit missing every earlier unjudged circle first); `drawablehitcircle.cs:137-173,277-306` (the receptor, the timing grade, the head's hit action); `sliderinputmanager.cs` (tracking, the accepted-key rule, `TryJudgeNestedObject`'s tail-after-last-tick rule, `PostProcessHeadJudgement`); `drawableslider.cs:293-326` (the slider's own ignore result); `drawablespinner.cs:236-365`/`spinnerrotationtracker.cs`/`spinnerspinhistory.cs` (progress, the grade at the end, one tick per completed spin); `hitresult.cs`/`scoreprocessor.cs:239-432` (the result vocabulary and the standardised fold, in `score::HitResult` and `simulation::native::scoring`); `osuhealthprocessor.cs`/`draininghealthprocessor.cs`/`healthprocessor.cs` (the native health: the increase table, the combo-end bonus, the drain-rate search over the perfect play, the break periods, the fail rule, in `simulation::native::health`). pinned element for element and on every end-state value by the no-mod dumps in `fixtures/judgement/`; the health by unit tests against the pinned constants, since the dumps carry no health curve |
...
//! | [`configuration`] | the play configuration resolved once per load: the rules profile by header version (`legacyscoreencoder.cs:74`), the effective mods from the score-info block or, as `legacyscoredecoder.cs:88,134` falls back, the legacy bitfield through the osu! ruleset's converter (`osuruleset.cs:77-129`), and the capabilities -- simulate, edit frames, regenerate export -- each allowed or refused with a reason authored there once. not a port beyond those citations: which configurations this crate simulates is its own matrix |
//! | [`formats::score_info`] | `osu.game/scoring/legacy/legacyreplaysoloscoreinfo.cs` (the schema), `apimod.cs` (a mod's acronym and non-default settings), `legacyscoreencoder.cs:132-152` (the lzma-over-json framing) and `jsonserializableextensions.cs` (the indented, snake-case, defaults-omitted shape lazer writes), pinned against the real export at `fixtures/replays/local/lazer-export.osr`. object key order is kept in document order, which lazer's dictionaries have and `serde_json::Value` would sort away |
...
//! | [`score::health`] | not a lazer port at all: stable's own HP system, read out of the client (`.scratch/stable-osr-writer/findings.md` Q1-Q2) with danser-go `app/rulesets/osu/healthprocessor.go` as the corroborating community port. lazer's nearest equivalent, `LegacyDrainingHealthProcessor.cs` + `OsuLegacyHealthProcessor.cs`, is knowingly incomplete (no combo-end additions, flat 50/100 gains, no preempt-adjusted first gap) and its graph writer returns `string.Empty`; the deliberate divergences from danser (floored preempt, stable's own slider point lists, the iteration cap) are stated at their code sites |
