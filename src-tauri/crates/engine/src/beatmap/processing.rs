...
use crate::beatmap::difficulty::{
    ...
    stable_radius_from_circle_size, OsuHitWindows,
};
...
pub struct ProcessedBeatmap {
    ...
    /// stable's circle radius for the simulation's click and follow tests
    /// (danser difficulty.go:126-127) -- rounds differently from `scale` in
...
pub struct ProcessedObject {
    ...
    /// stable's effective new-combo flag, the section boundary geki/katu
    /// derive under (danser parser.go:360-372): the raw flag, with the first
    /// object after a spinner forced true; a spinner keeps its raw flag
    /// untouched. distinct from `combo_index`, which follows lazer's
...
pub struct ProcessedSlider {
    ...
    /// stable's score points for the legacy simulation path (engine parity
    /// pass, issue 13) -- generated by [`crate::beatmap::stable_points`],
    /// which differs from the lazer nested list above in tick existence,
    ...
    /// `ProcessedObject::end_time` stays lazer's for everything else
    pub stable_end_time: f64,
}
...
pub fn process_beatmap(map: &Beatmap) -> Result<ProcessedBeatmap> {
    ...
    apply_stable_new_combo(map, &mut objects);
...
/// stable's load-time new-combo forcing (danser parser.go:360-372): every
/// spinner arms a force that lands on the next NON-spinner object; the
/// spinner's own flag stays exactly what the file said. geki/katu section
/// boundaries (score::sections) read this flag, never lazer's enforcement
fn apply_stable_new_combo(map: &Beatmap, objects: &mut [ProcessedObject]) {
    let mut force = false;
    for (i, obj) in objects.iter_mut().enumerate() {
        let raw = map.hit_objects[i].new_combo;
        if matches!(obj.kind, ProcessedKind::Spinner(_)) {
            obj.stable_new_combo = raw;
            force = true;
        } else {
            obj.stable_new_combo = raw || force;
            force = false;
        }
    }
}
...
mod tests {
    ...
    fn total_nested_object_cap_boundary() {
        ...
        let map = tickless(base_map(vec![
            linear_slider(0.0, Vec2::ZERO, 100.0, r1 as i32),
            linear_slider(1e9, Vec2::ZERO, 100.0, r2 as i32),
        ]));
